...
description: Safely install, check, update, repair, roll back, or uninstall Product OS and verify configured existing MCP capabilities.
capabilities:
  - transcript.search
...
1. Resolve the public project URL to an immutable commit. Show origin, publisher, commit, the plan's `source_state`, release manifest, and expected content hashes. When `source_state` is not `clean`, state that the commit does not describe what will be installed and let the human decide before continuing. A fork or alternate origin requires explicit human confirmation.
...
4. Preview the exact resulting `.product-os/config.yaml` values and their consequences inside the plan-hash preview. Explicitly state that local Git cannot prove provider review state, and that solo approval is unavailable unless the policy allows it. Save a validated configuration input outside the target repository. Only the deterministic installer may write the target copy.
...
6. Invoke the provenance-verified deterministic installer in `preview` mode with `--write-plan` using parameterized paths:
   ...
   ```text
   uvx --reinstall --from <verified-source> product-os-install \
...
8. The validated plan installs canonical assets under `.product-os/` and every generated route-only wrapper `SKILL.md` file for the stored selected client using these allowlisted roots:
...
12. Show the exact proposed Git diff and credential/privacy scan. The apply confirmation covers the target, configuration, plan hash, and the proposed standardized commit; never push without explicit user authority.

## Check, update, and recovery verbs

- **Check:** if `.product-os/` is absent, ask whether the workspace is at another path. Otherwise run `uvx --reinstall --from <verified-source> product-os check <target>`; report version + source commit, Managed integrity, wrappers as byte-verified, workspace validity, and connector states. State that live discovery begins in the next workspace-root session. If the recorded source commit is unavailable, instruct the user to re-clone the origin and `git checkout <recorded-commit>`.
- **Update:** trust the new source as for install, then use the same saved v2 plan/hash discipline. The planner reads the installed config, requires a committed baseline and clean Managed planned paths, and never touches product artifacts, Preserved files, or user-owned paths. Preview old → new commit, action/ownership counts, conflicts, and the reassurance `Product artifacts and configuration remain unchanged.`
- **Repair:** preview restoration of only named Managed conflicts to the installed-manifest baseline from a verified checkout at the recorded source commit. Offer exactly repair then re-run update, or stop and keep local edits. Never touch Preserved files.
- **Rollback:** preview one `git revert` of the named install/update commit, then require confirmation.
- **Uninstall:** when never used (no `product/` files, no `context/strategy.md`, context files unchanged since install), preview reverting the install commit. Otherwise preview deletion of Managed and Generated manifest paths, preserve product/context/config/context files, and offer to strip dangling Product OS routing from `AGENTS.md`/`CLAUDE.md`; require one confirmation and one commit.
