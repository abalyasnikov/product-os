...
def canonical_digest(repo_root: Path) -> str:
    files = sorted(
        path
        for base in (repo_root / "skills", repo_root / "integrations")
...
def test_client_adapters_have_matching_generated_metadata(repo_root: Path) -> None:
    expected_hash = canonical_digest(repo_root)
    seen_hashes = set()
    ...
    for client, install_location in CLIENTS.items():
        ...
        assert manifest["projection"]["wrapper_root"] == "skills"
...
def test_manifests_define_exact_source_wrapper_and_destination(repo_root: Path) -> None:
    for client, install_root in CLIENTS.items():
        ...
        for item in manifest["projections"]:
            ...
            assert item["wrapper_source"] == f"adapters/{client}/skills/{item['name']}/SKILL.md"
...
def test_wrapper_content_is_client_neutral(repo_root: Path) -> None:
    for name in ROUTES:
        contents = {
            (repo_root / f"adapters/{client}/skills/{name}/SKILL.md").read_text(
                encoding="utf-8"
            )
            for client in CLIENTS
        }
