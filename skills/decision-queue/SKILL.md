...
4. Surface only: evidence gaps/waivers; Opportunities awaiting `pursue|hold|reject`; Initiative/PRD reviews; renewed review after material change; evidence/Learnings challenging active assumptions; missing measurement anchors; due measurement windows; draft Learnings awaiting `scale|iterate|hold|kill|complete`; a `context/strategy.md` whose `review_by` date has passed; and blocking connector failures.

   Expired strategy is one queue item for the workspace, never one per affected artifact. Its decision is whether the strategy still holds, and until the human answers, every downstream strategic-fit judgment rests on a document nobody has confirmed. Do not silently keep applying it and do not stop the workflows that read it.
